...
export function PlayableBalanceCard({ balanceLabel, fundingEnabled = false, compact = false, onNavigate }: {
  ...
  return <article aria-label="Playable Wallet" className={`h-full min-w-0 rounded-2xl border border-cyan-300/20 bg-[#0c3b5d] ${compact ? "grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-2 p-3" : "flex flex-col p-4"}`}>
    ...
    <div className={compact ? "contents" : "mt-auto flex flex-wrap items-center justify-between gap-2 pt-2"}>
      ...
      {!compact ? <Link href={walletHistoryHref} onClick={onNavigate} className="inline-flex min-h-11 items-center gap-1 rounded-lg text-sm font-bold text-cyan-300 hover:text-white focus-visible:outline-2 focus-visible:outline-cyan-300">View transactions <span aria-hidden="true">›</span></Link> : null}
    </div>
  </article>;
}
...
export function MyZeroLossSummary({ state, compact = false, onNavigate }: { state: AccountActivity; compact?: boolean; onNavigate?: () => void }) {
  ...
  return <Link href="/account/entries" onClick={onNavigate} aria-label="My Zero Loss — See every entry and outcome" data-activity-source={state.source}
    ...
    <h2 className={`text-xs font-black uppercase tracking-[0.14em] text-cyan-300 ${compact ? "col-start-1 row-start-1" : ""}`}>My Zero Loss</h2>
    ...
    <p className={`mt-1 leading-5 text-[#b5cce4] ${compact ? "col-span-2 row-start-3 text-xs" : "text-sm"}`}>{counts ? `${counts.open} open · ${counts.won} won · ${counts.options} purchase ${counts.options === 1 ? "option" : "options"}` : "We can’t verify your activity right now."}</p>
    <span className={`${compact ? "col-start-2 row-start-1 row-span-2 text-xs" : "mt-auto block pt-2 text-sm"} font-bold text-cyan-300 group-hover:text-white`}>View activity <span aria-hidden="true">→</span></span>
  </Link>;
}
