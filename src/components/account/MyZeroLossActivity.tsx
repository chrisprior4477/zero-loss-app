...
export function MyZeroLossActivity({ state, filter, selectedSlug }: { state: AccountActivity; filter: ActivityFilter; selectedSlug?: string }) {
  ...
  return <section data-activity-source={state.source}>
    ...
    {state.isPreview ? <p className="mt-4 text-xs leading-5 text-[#b5cce4]">Illustrative catalog activity, not stored entries or issued rewards. These examples do not change your database balance.</p> : null}
