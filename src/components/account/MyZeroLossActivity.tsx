...
export function MyZeroLossActivity({ state, filter, selectedSlug, selectedEntryId }: { state: AccountActivity; filter: ActivityFilter; selectedSlug?: string; selectedEntryId?: string }) {
  ...
  return <section data-activity-source={state.source}>
    ...
    {items.length ? <MyZeroLossGallery key={filter + items.map(item => item.entryId ?? item.slug).join(",")} items={items} filter={filter} /> : <div className={styles.empty}>
    ...
    <ActivitySelection state={verifiedState} selectedSlug={selectedSlug} selectedEntryId={selectedEntryId} destination="/account/entries" filter={filter} />
  </section>;
}
