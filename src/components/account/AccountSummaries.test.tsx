...
test("normal empty and unavailable summaries never substitute fixtures or invent zeros", () => {
  ...
  expect(screen.getByRole("link", { name: "Your wallet — 0 rewards" }).getAttribute("href")).toBe("/account/wallet");
...
test("multiple rewards lead to the collection instead of choosing a reward for the customer", () => {
  ...
  expect(screen.getByRole("link", { name: "Your wallet — 2 sample rewards" }).getAttribute("href")).toBe("/account/wallet");
});
...
test("dashboard orders the summaries and moves the saved photo into account navigation", async () => {
  ...
  expect(overview.getAllByRole("heading").map(h => h.textContent)).toEqual(["Playable balance", "Your wallet", "My Zero Loss"]);
