...
export function WalletOverview({ wallet, previewAuthorized, fundingEnabled = false, requestKey = "", requests = null }: { wallet: WalletSnapshot | null; previewAuthorized: boolean; fundingEnabled?: boolean; requestKey?: string; requests?: DemoFundingRequest[] | null }) {
  ...
  return <div className="mt-7 grid items-start gap-6 lg:grid-cols-[minmax(260px,0.8fr)_minmax(0,1.4fr)]">
    <section id="demo-funding" className="scroll-mt-48 rounded-3xl border border-[#31e800]/20 bg-[linear-gradient(135deg,#073453,#063c38)] p-6 sm:p-7" aria-label="Playable balance">
      <h2 className="text-xs font-black uppercase tracking-widest text-[#b5cce4]">{demo ? "Demo Playable Balance" : "Playable Balance"}</h2>
      ...
      <p className="mt-3 text-sm font-bold text-white">Funding is not enabled yet.</p>
      <p className="mt-2 text-xs leading-5 text-[#b5cce4]">{previewAuthorized ? "Read-only preview permission does not create a demo wallet or allow deposits. No payment will be processed." : "You can review your balance and existing transactions. No payment will be processed."}</p></>}
    </section>
