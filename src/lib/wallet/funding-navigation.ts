...
export function fundingHref(productSlug?: string, entryId?: string): string {
...
export function fundingReturnPath(value: unknown): string | null {
  ...
  return /^\/account\/wallet\?view=history(?:&from=[a-z0-9]+(?:-[a-z0-9]+)*(?:&entry=ent_[a-f0-9]{32})?)?#add-funds$/.test(value)
    ? value
    : null;
}
