...
$checkpointVersion = '20260917233000'
...
function Get-CheckpointTest([string]$TestName = 'customer_profile_contact_details_test.sql') {
  $checkpointTest = Get-Content -Raw -LiteralPath (Join-Path $checkpointRoot "supabase/tests/$TestName")
  $checkpointHarness = @'
...
try {
  ...
  if ($Mode -eq 'Test') {
    [pscustomobject]@{
      contact_details = Invoke-CheckpointQuery (Get-CheckpointTest)
      profile_security_regression = Invoke-CheckpointQuery (Get-CheckpointTest 'customer_profiles_write_path_test.sql')
    } | ConvertTo-Json -Depth 6
    return
  }
  if ($Mode -eq 'Verify') {
    Invoke-CheckpointQuery "select exists(select 1 from supabase_migrations.schema_migrations where version='$checkpointVersion') as migration_applied, count(*) filter (where phone_number is not null or address_line_1 is not null) as profiles_with_contact_details from public.customer_profiles;" | ConvertTo-Json
    return
  }

  $checkpointSource = Get-Content -Raw -LiteralPath (Join-Path $checkpointRoot "supabase/migrations/${checkpointVersion}_customer_profile_contact_details.sql")
...
do `$$ begin
  if exists(select 1 from supabase_migrations.schema_migrations where version='$checkpointVersion') then raise exception 'Profile-details migration already applied'; end if;
  if not exists(select 1 from supabase_migrations.schema_migrations where version='20260917230000') then raise exception 'Previous saved-card checkpoint missing'; end if;
...
"@
  if ($Mode -eq 'DryRun') {
    ...
    Invoke-CheckpointQuery ($checkpointGuard + "`n" + $checkpointBody + "`ninsert into supabase_migrations.schema_migrations(version,name,statements) values ('$checkpointVersion','customer_profile_contact_details',ARRAY[`$checkpoint_source`$$checkpointSource`$checkpoint_source`$]);`ncommit;") | ConvertTo-Json
  }
