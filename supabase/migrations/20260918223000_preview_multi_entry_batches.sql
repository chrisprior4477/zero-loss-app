-- Restore multi-entry selection with one immutable financial record per entry.
begin;

alter table public.customer_entries
  drop constraint if exists customer_entries_wallet_account_id_offering_slug_key;

create table public.preview_entry_batches (
  id uuid primary key default gen_random_uuid(),
  batch_id text not null unique check (batch_id ~ '^bat_[0-9a-f]+$'),
  ...
  unique (id, wallet_account_id, customer_id, wallet_scope),
...
alter table public.customer_entries add column preview_batch_id uuid;
alter table public.customer_entries add constraint customer_entries_preview_batch_identity
  foreign key (preview_batch_id, wallet_account_id, customer_id, wallet_scope)
  references public.preview_entry_batches(id, wallet_account_id, customer_id, wallet_scope) on delete restrict;
create index customer_entries_preview_batch_idx
  on public.customer_entries(preview_batch_id, created_at, id)
  where preview_batch_id is not null;

alter table public.preview_entry_batches enable row level security;
revoke all on public.preview_entry_batches from public, anon, authenticated, service_role;
grant select on public.preview_entry_batches to authenticated;
create policy "Customers can view own preview entry batches" on public.preview_entry_batches
for select to authenticated using (
  customer_id = (select auth.uid())
  and wallet_account_id = (select public.current_wallet_account_id())
);

create trigger preview_entry_batches_immutable before update or delete on public.preview_entry_batches
  for each row execute function public.reject_financial_history_mutation();
create trigger preview_entry_batches_no_truncate before truncate on public.preview_entry_batches
  for each statement execute function public.reject_financial_history_mutation();

create function demo_private.preview_entry_batch_response(p_batch_id uuid, p_duplicate boolean default false)
...
create function public.create_preview_entries(
...
$$;
revoke all on function public.create_preview_entries(text,integer,text)
  from public, anon, authenticated, service_role;
grant execute on function public.create_preview_entries(text,integer,text) to authenticated;

create or replace function public.create_preview_entry(p_offering_slug text, p_idempotency_key text)
returns jsonb language sql security definer set search_path = '' as $$
  select public.create_preview_entries(p_offering_slug, 1, p_idempotency_key);
$$;
